...
class TestConstructor:
    ...
    def test_open_fillings_tab(self, driver):
        ...
        # Ждём, пока вкладка "Начинки" станет активной
        WebDriverWait(driver, 5).until(
            EC.visibility_of_element_located(ConstructorPageLocators.ACTIVE_FILLINGS_TAB)
        )

        # Проверяем, что вкладка "Начинки" активна
        assert driver.find_element(*ConstructorPageLocators.ACTIVE_FILLINGS_TAB).is_displayed()
